...
const FSSAIVerification = ({ initialTab = 'verification' }) => {
  const { user } = useAuth();
  const [selectedService, setSelectedService] = useState(initialTab);
  ...
  const handleRegister = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:5002/api/fssai/register', formData, {
        headers: {
